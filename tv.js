...
function setupSearch() {
    ...
    searchInput.addEventListener("input", (e) => {
        ...
        console.log(`⌨️ كتابة في المسلسلات: "${currentSearchQuery}"`);
        ...
        autocompleteTimeout = setTimeout(async () => {
            console.log(`🔍 بحث عن مسلسل: "${currentSearchQuery}"`);
    ...
    searchInput.addEventListener("keypress", (e) => {
        if (e.key === 'Enter') {
            ...
            if (query.length > 0) {
                console.log(`↵ Enter: بحث مباشر عن مسلسل "${query}"`);
                searchSeries(query);
                hideAutocomplete();
            }
        }
...
async function fetchAutocompleteSuggestions(query) {
    ...
    try {
        console.log(`🌐 جلب اقتراحات مسلسلات لـ: "${query}"`);
        const url = `${BASE_URL}/search/tv?api_key=${API_KEY}&language=ar&query=${encodeURIComponent(query)}&page=1`;
        const res = await fetch(url);
        ...
        const data = await res.json();
        console.log(`✅ تم جلب ${data.results?.length || 0} اقتراح مسلسل`);
        ...
        console.error("❌ خطأ في جلب اقتراحات المسلسلات:", error);
        return [];
    }
}
...
function showAutocomplete(suggestions, query) {
    ...
    suggestions.forEach(series => {
        const title = series.name || "بدون عنوان";
        const year = series.first_air_date ? series.first_air_date.substring(0, 4) : "غير معروف";
        const rating = series.vote_average ? series.vote_average.toFixed(1) : "N/A";
        const posterUrl = series.poster_path ? `https://image.tmdb.org/t/p/w92${series.poster_path}` : 
        ...
        html += `
            <div class="suggestion-item" onclick="selectSuggestion(${series.id}, '${safeTitle}', 'series')">
                <img src="${posterUrl}" alt="${title}" loading="lazy">
                <div class="suggestion-info">
                    <div class="suggestion-title">${highlightedTitle}</div>
                    <div class="suggestion-year">
                        ${year} • ${series.original_language?.toUpperCase() || 'N/A'}
                        ${seasons > 0 ? ` • ${seasons} ${seasons === 1 ? 'موسم' : 'مواسم'}` : ''}
                    </div>
                </div>
    ...
    container.classList.add("show");
    console.log(`✅ عرض ${suggestions.length} اقتراح مسلسل`);
}
...
// دالة اختيار اقتراح
function selectSuggestion(seriesId, seriesTitle, type = 'series') {
    console.log(`📺 اختيار ${type === 'series' ? 'مسلسل' : 'فيلم'}: ${seriesTitle} (ID: ${seriesId})`);
    
    const searchInput = document.getElementById("search");
    if (searchInput) {
        searchInput.value = seriesTitle;
    }
    
    hideAutocomplete();
    searchSeries(seriesTitle);
}

// ========================================
// UPDATED SEARCH SERIES FUNCTION
// ========================================
...
async function searchSeries(query) {
    ...
    try {
        showProgress();
        console.log(`🔍 بحث TMDb عن مسلسل: "${query}"`);
        ...
        const data = await res.json();
        console.log(`✅ وجدنا ${data.results?.length || 0} نتيجة مسلسل`);
        ...
        console.error("❌ خطأ في البحث عن المسلسلات:", error);
...
// ========================================
// INITIALIZE AUTOCOMPLETE FOR TV
// ========================================

function initTVAutocomplete() {
    console.log("🔧 تهيئة نظام الاقتراحات التلقائية للمسلسلات...");
    
    const searchInput = document.getElementById("search");
    if (!searchInput) {
        console.error("❌ لم يتم العثور على حقل البحث في صفحة المسلسلات!");
        return;
    }
    
    // التحقق من وجود حاوية الاقتراحات
    if (!document.getElementById("autocomplete-suggestions")) {
        console.log("📦 إنشاء حاوية الاقتراحات للمسلسلات...");
        ...
        if (searchContainer) {
            ...
            console.log("✅ تم إنشاء حاوية الاقتراحات للمسلسلات");
        } else {
            console.error("❌ لم يتم العثور على حاوية autocomplete-container في صفحة المسلسلات");
        }
    }
    
    console.log("✅ نظام الاقتراحات التلقائية للمسلسلات جاهز");
}

// ========================================
// UPDATE initTVPage FUNCTION
// ========================================

function initTVPage() {
    console.log("⚙️ بدء تحميل صفحة المسلسلات");
    initTVAutocomplete();  // أضف هذا السطر أولاً
    setupSearch();         // ثم استدعاء setupSearch
    updateWatchlistCounter();
    loadAllSeries();
    
    // اختبار النظام
    console.log("🧪 اختبار نظام المسلسلات:");
    ...
    console.log("- نظام البحث عن المسلسلات جاهز!");
}
...
function fixBanner() {
    console.log("🔧 إصلاح بانر المسلسلات...");
    
    const cards = document.querySelectorAll('.banner-card');
    console.log(`🔍 عدد بطاقات البانر: ${cards.length}`);
    ...
    if (cards.length > 0) {
        ...
        console.log("✅ تم إصلاح بانر المسلسلات");
    }
}
...
// كود اختبار يدوي
window.testTVSearch = function() {
    const input = document.getElementById('search');
    if (input) {
        input.value = "مسلسل";
        input.dispatchEvent(new Event('input', { bubbles: true }));
        console.log("✅ تم تشغيل اختبار البحث التلقائي للمسلسلات");
    } else {
        console.error("❌ لم يتم العثور على حقل البحث في صفحة المسلسلات");
    }
};

// اطبع معلومات التصحيح في الكونسول
console.log("ℹ️ معلومات التصحيح لصفحة المسلسلات:");
console.log("API_KEY موجود:", !!API_KEY);
console.log("BASE_URL:", BASE_URL);
console.log("الرابط الكامل للبحث عن المسلسلات:", `${BASE_URL}/search/tv?api_key=${API_KEY}&language=ar&query=test`);